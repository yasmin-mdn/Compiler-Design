...
expression returns [node=None]
    ...
    |'this' {$node = self.ast.make_node('this','None','None')}
    | 'new' 'int' '[' e=expression ']' {$node = self.ast.make_node('array-initialize',$e.node,None)}
    | 'new' i=identifier '(' ')' {$node = self.ast.make_node('object-initialize',$i.node,None)}
    | '!' e=expression {$node = self.ast.make_node('not',$e.node,None)}
    | '(' e=expression ')' {$node = self.ast.make_node('cast-object',$e.node,None)}
    ;
